...
if (!prodEnvironment) console.log(`Running in ${prodEnvironment ? 'production' : 'development'} environment.`, process.env.USER)
...
export default async function fetchCurrencyConversion() {
    ...
    try {
        ...
        if (!prodEnvironment) console.log(`Successfully saved currency conversion to ${targetFile}`)
    } catch (error) {
...
if (process.argv[1] === fileURLToPath(import.meta.url)) {
    fetchCurrencyConversion().then(() => {
        if (!prodEnvironment) console.log('Done.')
        process.exit(0)
    })
}